  ...
  methods: {
    ...
    postSaveProcessing: function postSaveProcessing(successResponse) {
      if (this.isJustCreated()) {
        // assign new Id
        var _id = parseInt(successResponse.id);

        this.id = _id; // change the address bar URL to en edit page url

        this.$router.replace({
          path: '/app/templates/' + _id
        });
      }
    ...
    editTemplateFile: function editTemplateFile(file) {
      this.$router.push({
        path: '/app/templates/' + id + '/files/' + file.name
      });
    }
  }
});
