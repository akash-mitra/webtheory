  ...
  methods: {
    ...
    fetchContentAndLoadEditor: function fetchContentAndLoadEditor() {
      ...
      if (typeof this.$route.params.id != 'undefined' && parseInt(this.$route.params.id) > 0 && typeof this.$route.params.fileIdentity != 'undefined') {
        this.fileName = atob(this.$route.params.fileIdentity);
        util.ajax('get', '/api/templates/' + this.$route.params.id + '/get/' + this.$route.params.fileIdentity, {}, function (data) {
          ...
          if (status === 404) {
            util.notifyError(message.status, this.fileName + ' can not be found.');
          } else {
            alert('Some error occurred: ' + message.status);
          }
        });
      }
    },
