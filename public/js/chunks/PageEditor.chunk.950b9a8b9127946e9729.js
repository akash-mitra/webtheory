...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    downContentBlock: function downContentBlock(index) {
      ...
      block['changed'] = true;
      var nextBlock = this.contents[index + 1]; // we will need to change the next block's
      // display Order, when its existing display order
      // is conflicting with this block's display order.

      if (nextBlock.display_order === block.display_order) {
        nextBlock.display_order -= 1;
    ...
    upContentBlock: function upContentBlock(index) {
      ...
      block.display_order -= 1;
      block['changed'] = true;
      var prevBlock = this.contents[index - 1]; // we will need to change the previous block's
      // display Order, when its existing display order
      // is conflicting with this block's display order.

      if (prevBlock.display_order === block.display_order) {
        prevBlock.display_order += 1;
          ...
          _c(
            ...
            [
              _c(
                ...
                {
                  staticClass:
                    "absolute top-0 right-0 text-gray-600 text-xs px-4 py-1 flex justify-between"
                },
                [
                  _c(
                    "svg",
