...
@section('contents')
    ...
    <div class="w-full max-w-6xl mx-auto px-6 font-reading">
        ...
        <div class="w-full md:flex bg-{{$data->ref->template->primaryColor}}-100 rounded-lg p-6 items-start">
            <img class="h-16 w-16 md:h-24 md:w-24 rounded-full mx-auto md:mx-0 md:mr-6 shadow-lg" src="{{ $data->profile->avatar }}">
            <div class="md:w-5/6 text-center md:text-left">
                <h2 class="text-2xl pb-211 inline-flex flex-col md:flex-row items-center">
                    <span>{{ $data->profile->name }}</span>
                    <span class="md:ml-2 px-1 py-1 text-center  rounded text-sm bg-{{$data->ref->template->primaryColor}}-200 text-{{$data->ref->template->primaryColor}}-600">{{ ucfirst($data->profile->role) }}</span>
                </h2>

                <div class="text-sm py-1">Joined {{ $data->profile->created_ago }}</div>

                <div class="py-2">{{ $data->profile->about_me }}</div>
            </div>
        </div>
