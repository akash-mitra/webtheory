...
<head>
  ...
  <link rel="canonical" href="{{ $data->page->url }}">
        ...
        <main class="w-full max-w-3xl @if($data->ref->template->centerContent === 'yes') mx-auto @endif py-10 px-6 text-gray-800">
            <article class="wt-typography">
                <header>
                    ...
                    <h1 class="text-gray-800 font-serif">{{ $data->page->title }}</h1>
                    <div class="w-full flex items-center mt-6 mb-8 text-sm">
                        {!! $data->page->author->photo() !!}
                        <div class="ml-4">
                            <span class="block">{{ $data->page->author->name }}</span>
                            <span class="block">{{ $data->page->updated_at->format('M d, Y') }}</span>
                        </div>
                    </div>

                    <h3 class="italic py-4 px-6 rounded bg-{{$data->ref->template->primaryColor}}-100 -mx-6">{{ $data->page->summary }}</h3>
                </header>
                <div class="mt-4">
