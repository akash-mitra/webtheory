    ...
    <div class="w-full sm:flex justify-between items-center p-4 border-b">
        ...
        <div class="w-1/6 sm:text-right mt-2 sm:mt-0">
            <span
                ...
                class="bg-red-100 px-2 text-red-600 py-1 rounded-lg text-sm"
            >
                Admin
            </span>
            ...
            <span
                ...
                class="bg-blue-100 text-blue-600 px-2 py-1 rounded-lg text-sm"
            >
                Author
            </span>
        ...
        <div v-if="canBan" class="w-full sm:w-1/6 flex justify-end sm:px-6">
            <div
                :id="'remove-ban-user-' + user.id"
                v-if="!!user.deleted_at"
                @click="$emit('unbanned', user.id)"
                class="tooltip cursor-pointer text-red-600 hover:text-red-400"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-4 w-4"
                ...
                <span class="tooltip-text bg-black text-white mb-2 rounded-lg text-xs py-1"
                    >Remove Ban</span
                >
            </div>
            ...
            <div
                :id="'ban-user-' + user.id"
                v-else
                @click="$emit('banned', user.id)"
                class="tooltip cursor-pointer text-gray-400 hover:text-red-600"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-4 w-4"
