<template>
    ...
    <div class="max-w-4xl mx-auto mb-12">
        ...
        <div class="px-6 w-full flex justify-start items-center my-8 rounded-lg">
            <div id="user-all-tab" @click="filterByTab('all')" class="px-6 py-2 rounded-l-full text-sm uppercase cursor-pointer" :class="tab==='all'? 'bg-white shadow-inner': 'border text-gray-500 bg-gray-100'">All</div>
            <div id="user-unverified-tab" @click="filterByTab('unverified')" class="px-6 py-2 rounded-none text-sm uppercase cursor-pointer flex items-center" :class="tab==='unverified'? 'bg-white shadow-inner': 'border text-gray-500 bg-gray-100'">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-3" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h0"/></svg>
                unverified
            </div>
            <div id="user-banned-tab" @click="filterByTab('banned')" class="px-6 py-2 rounded-r-full text-sm uppercase cursor-pointer flex items-center" :class="tab==='banned'? 'bg-white shadow-inner': 'border text-gray-500 bg-gray-100'">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-3" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M5 5l14 14"/></svg>
                banned
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            banUser (id) {
                ...
                util.confirm ("Ban this user?", "Once banned, the user won't be able to login.", function () {

                    util.ajax ('delete', '/api/users/' + id, {}, (response) => {

                        util.notifySuccess ('User Banned', 'The user has been banned from loggin-in to the site.')
