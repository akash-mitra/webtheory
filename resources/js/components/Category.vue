    ...
    <div v-show="needle === '' || (needle !== '' && (hasNeedle || childHasNeedle > 0))" class="">
        <div class="flex items-center">
            <svg
                ...
                viewBox="0 0 512 512"
                class="h-6 w-6 fill-current text-blue-300 mx-1"
            >
                <path
                    d="M511 316l-2-4-128-128a11 11 0 00-15 16l110 109H267A246 246 0 0121 64a11 11 0 00-21 0c0 147 120 267 267 267h209L366 440a11 11 0 0015 16l128-128 2-4v-8z"
                />
            </svg>

            <div
                class="p-4 w-full flex items-start justify-start rounded border"
                :class="hasNeedle ? 'bg-yellow-200' : 'bg-white'"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="text-right flex-grow-0 px-6">
                    <a
                        :id="'view-category-' + item.id"
                        :href="'/categories/' + item.id"
                        class="text-xs text-blue-800 px-3 py-1 bg-indigo-100"
                        >View</a
                    >
                </div>
            </div>
        ...
        <div v-show="item.children.length > 0" :class="level > 1 ? 'pl-10' : ''">
            <Category
                ...
                @NeedleFound="onNeedleFoundAtChild"
            ></Category>
        </div>
    </div>
</template>
