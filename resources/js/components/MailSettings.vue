        ...
        <div class="rounded max-w-xl1 bg-white shadow">
            ...
            <div class="p-6">
                <div class="w-full sm:flex mt-2">
                    <label
                        for="mailDriver"
                        class="block w-full sm:w-1/4 text-right mr-4 text-sm py-1"
                        >Mail Driver</label
                    >
                    <div class="inline-block relative w-64">
                        <select
                            ...
                            v-model="mailDriver"
                            ref="mailDriver"
                            class="block appearance-none w-full bg-gray-200 focus:bg-white px-2 py-1 pr-8 rounded border focus:outline-none"
                        >
                            <option
                                v-for="mailDriver in mailDrivers"
                                :value="mailDriver.key"
                                :key="mailDriver.key"
                                v-text="mailDriver.value"
                            ></option>
                        </select>
                        <div
                ...
                <div v-if="showSmtp" class="w-full sm:flex mt-2">
                    <label for="mailHost" class="block w-full sm:w-1/4 text-right mr-4 text-sm py-1"
                        >Mail Host</label
                    >
                    <input
                        type="text"
                ...
                <div v-if="showSmtp" class="w-full sm:flex mt-2">
                    <label for="mailPort" class="block w-full sm:w-1/4 text-right mr-4 text-sm py-1"
                        >Mail Port</label
                    >
                    <input
                        type="number"
                ...
                <div v-if="showSmtp" class="w-full sm:flex mt-2">
                    ...
                    <div class="inline-block relative w-64">
                        ...
                        >
                            <option
                                v-for="mailEncryption in mailEncryptions"
                                :value="mailEncryption.key"
                                :key="mailEncryption.key"
                                v-text="mailEncryption.value"
                            ></option>
                        </select>
                        <div
                            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700"
                ...
                <div v-if="showSmtp" class="w-full sm:flex mt-2 mb-4">
                    ...
                    <div class="w-full block sm:w-3/4 max-w-lg">
                        ...
                        <p class="mt-3 text-sm text-gray-700" v-show="mailHost.includes('gmail')">
                            For Gmail, you should not use your regular Gmail password above.
                            Instead, please use App specific password. Get your
                            <a
                                href="https://support.google.com/accounts/answer/185833"
                                target="_blank"
                                class="text-blue-500 hover:underline"
                                >Gmail App Password</a
                            >
                            here.
                        </p>
                    </div>
                ...
                <div v-if="showPostmark" class="w-full sm:flex mt-2 mb-4">
                    <label
                        for="postmarkToken"
                        class="block w-full sm:w-1/4 text-right mr-4 text-sm py-1"
                        >Postmark Token</label
                    >
                    <input
                        type="text"
                        id="postmarkToken"
                        v-model="postmarkToken"
                        ref="postmarkToken"
                        class="w-full sm:w-3/4 max-w-lg px-2 py-1 rounded appearance-none bg-gray-200 focus:bg-white border focus:outline-none"
                    />
                </div>
                ...
                <div v-if="showSes" class="w-full sm:flex mt-2">
                    <label for="sesKey" class="block w-full sm:w-1/4 text-right mr-4 text-sm py-1"
                        >SES Key</label
                    >
                    <input
                        type="text"
                ...
                <div class="w-full sm:flex mt-2 mb-4">
                    ...
                    <div class="w-full sm:w-3/4 max-w-lg py-3 flex items-center justify-end">
                        <button
                            v-if="mailSaved"
                            @click="testMail"
                            class="px-3 py-1 mr-3 border border-blue-400 rounded text-blue-500 bg-transparent"
                        >
                            Send Test Email
                        </button>
                        <t-button
